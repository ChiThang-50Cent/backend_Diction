            ...
            for await (const word of list) {
                ...
                const result = await page.evaluate(() => {
                    ...
                    if (node) {
                        ...
                        const phonetic = document.querySelector('#entryContent .phonetics span.phon').textContent
                        const soundMp3 = node.querySelector('.phonetics div.sound').getAttribute('data-src-mp3')
                        const soundOgg = node.querySelector('.phonetics div.sound').getAttribute('data-src-ogg')
